...
-- Workout sessions (flexible rep logging)
...
BEGIN
  ...
  IF v_last_workout_date = CURRENT_DATE THEN
    UPDATE user_streaks SET total_workouts = total_workouts + 1, updated_at = NOW() WHERE user_id = p_user_id;
  ELSIF v_last_workout_date = CURRENT_DATE - INTERVAL '1 day' THEN
    ...
    SET current_streak = v_current_streak, longest_streak = v_longest_streak,
        last_workout_date = CURRENT_DATE, total_workouts = total_workouts + 1, updated_at = NOW()
    WHERE user_id = p_user_id;
  ELSE
    UPDATE user_streaks
    SET current_streak = 1, last_workout_date = CURRENT_DATE, total_workouts = total_workouts + 1, updated_at = NOW()
    WHERE user_id = p_user_id;
  END IF;
END;
